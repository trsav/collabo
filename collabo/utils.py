...
def distribute_solutions(fixed_solutions,bounds,required):
    ...
    remaining_solutions = np.random.uniform(size=(required*len(bounds))) # random initial guess

    print(f'Distributing remaining {required} solutions')

    res = opt.minimize(obj,remaining_solutions,args=(fixed_solutions),method='Nelder-Mead',bounds=[[0,1] for i in range(required*len(bounds))],options={'disp':True,'maxiter':10000})
...
class GP(gpytorch.models.ExactGP):
    def __init__(self, train_x, train_y, likelihood):
        ...
        self.covar_module = gpytorch.kernels.ScaleKernel(gpytorch.kernels.RBFKernel(ard_num_dims=len(train_x[0])))
